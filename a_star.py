...
def calc_final_path(ngoal, closedset, reso):
    ...
    rz, tz = [ngoal.z], [terrain[round(ngoal.x), round(ngoal.y)]]
    pind = ngoal.pind
    if show_animation:
    ...
    while pind != -1:
        ...
        ry.append(n.y * reso)
        rz.append(n.z)
        tz.append(terrain[int(n.x), int(n.y)])
        pind = n.pind
...
def dp_planning(sx, sy, gx, gy, ox, oy, reso, rr, max_g):
    ...
    while 1:
        ...
        for i, _ in enumerate(motion):
            ...
            if abs_td > motion[i][2]*max_g:
                ...
                csc = (max_g**2+1**2)**0.5  # 余切
            else:
                new_z = terrain[new_x, new_y]
                csc = ((new_z-current.z)**2 +
                       (motion[i][2])**2)**0.5/(motion[i][2])  # 余切
            # 梯形体积计算
            volumn = abs((terrain[new_x, new_y]-new_z)+(
    ...
    rx, ry, rz, tz = calc_final_path(closedset[calc_index(
    ...
    return rx, ry, closedset, Z, rz, tz
...
def main():
    ...
    rx, ry, closedset, Z, rz, tz = dp_planning(
    ...
    plt.figure()
    plt.plot(range(len(rz)), rz, "-r")
    plt.plot(range(len(tz)), tz, "-k")
    plt.show()
    # fig = plt.figure()
