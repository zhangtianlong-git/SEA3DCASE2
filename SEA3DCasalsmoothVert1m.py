...
class Node:
    ...
    def get_pre_constrain_inf(self):
        ...
        self.enough_pre_slope_len_horizontal = self.seqSS >= self.pre_ver_cur_len
        # 水平直线长度约束
...
def get_neighbors(current, config):
    ...
    if pe:
        ...
        for i in pe:
            ...
            if (
                round(x) < 0
                or round(x) >= config.xw - 1
                or round(y) < 0
                or round(y) >= config.yw - 1
            ):  # 检查越界
                continue

            if abs(terrain[round(x), round(y)] - z) > 3.5:  # 最大挖填方深度
                continue
...
def check_not_in_two_points(x1, y1, theta, x, y):  # 判断终点是否在最后两个点之间
    dx, dy = x - x1, y - y1
    dx1, dy1 = 1, math.tan(theta)
    if dx * dx1 + dy * dy1 > 0:
        return True
    else:
        return False
...
heapq.heappush(pq, (tf.calc_cost_new(nstart, heuristic), tf.calc_index(nstart, config)))
num_iter = 0
...
while True:
    ...
    if (
        ...
        if (
            ...
            and (delta_z < 0.4)
            and (check_not_in_two_points(cx, cy, ctheta, gx, gy))
        ):
            is_exceed, cost, c_s_cost, loc_len_cost, f_c_list = get_cost(
